...
import read_email
import add_company_to_portfolio
import check_score_available
import generate_company_summary_report
import send_email
...
def main():
    """
    Main function that calls all other scripts in Security_Scorecard_APIs program.
    Triggered by Outlook VBA macro when email arrives in Inbox within specified
    filters (sent time and subject line).
    """
    try:
        # get company website
        company = read_email.get_website()

        # add company website to portfolio (needed for generating reports)
        add_company_to_portfolio.add_company(company)

        # check if score is available
        score_ready = check_score_available.check_score(company)

        print(score_ready)
        # only generate and download report if the score is ready
        if score_ready:
            # generate report & get report ID, return boolean whether report was found
            report, report_found = generate_company_summary_report.get_report_url(company)
        
            # use report ID to get download link, send link in email
            send_email.send_report(report, report_found)

    except Exception as err:
        # print error message and insert message into body of email
        err_msg = "An exception of type {0} occurred. Arguments:\n{1!r}"
        message = err_msg.format(type(err).__name__, err.args)
        print(message)
        #if 'company must be added to a portfolio first' in message:

        
        # send email with error message
        mail = outlook.CreateItem(0)
        mail.To = 'pwalsh@syscom.com'
        mail.Subject = 'REPORT ERROR!'
        mail.Body = message
        mail.Send()
